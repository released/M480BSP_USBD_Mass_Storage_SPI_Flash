    ...
    if (g_u32OutSkip == 0)
    {
        if (g_u32Length > EP3_MAX_PKT_SIZE)
        {
            ...
            if (g_u32Address >= (STORAGE_DATA_BUF + STORAGE_BUFFER_SIZE))
            {
                ...
                #else
            	SpiWrite(g_u32DataFlashStartAddr, STORAGE_BUFFER_SIZE, (uint32_t)STORAGE_DATA_BUF);
                #endif
        ...
        else
        {
            ...
            if ((g_sCBW.u8OPCode == UFI_WRITE_10) || (g_sCBW.u8OPCode == UFI_WRITE_12))
            {
                ...
                if (len)
                {
                    ...
                    #else
                    SpiWrite(g_u32DataFlashStartAddr, len, (uint32_t)STORAGE_DATA_BUF);
                    #endif
                }
            }
...
void MSC_ReadMedia(uint32_t addr, uint32_t size, uint8_t *buffer)
{
    ...
    #else
    SpiRead(addr, size, (uint32_t)buffer);
    #endif
}
...
void MSC_WriteMedia(uint32_t addr, uint32_t size, uint8_t *buffer)
{
    ...
    #else
    SpiWrite(addr, size, (uint32_t)buffer);
    #endif
}
